...
void delay_ms(uint32_t ms) {
    /* Usa una función de la biblioteca SDK para crear un delay en milisegundos */
    DelayMs(ms);
}
	...
	switch (ledx){
	          case 1:
	        	  dir= MAXM86161_REG_LED1_PA;
				  	  	  maxm86161_i2c_write_to_register(dir,value);
				  	  	  break;
	          case 2:
...
{  uint8_t int_status_value;
  ...
  uint8_t dir;
   ...
   maxm86161_i2c_read_from_register(dir, &int_status_value);
                                               ...
                                               maxm86161_ppg_sample_t *sample)
{
  ...
  for ( i= 0; i < MAXM86161DRV_PPG_SAMPLE_SIZE_BYTES; i++) {
