...
/*===========================================================================*/
static void maxm86161_hrm_adjust_led_current(maxm_hrm_handle_t *handle,
                                             int32_t channel, int32_t direction);
static void maxm86161_hrm_perform_agc(maxm_hrm_handle_t *handle);
static void maxm86161_hrm_perform_dc_sensing(maxm_hrm_handle_t *handle,
                                             maxm86161_hrm_irq_sample_t *ppg);
static int32_t maxm86161_hrm_initialize_buffers(maxm_hrm_handle_t *handle);
static int32_t maxm86161_hrm_bpf_filtering(maxm_hrm_handle_t *handle,
                                           int32_t PS_input,
                                           int16_t *pPS_output,
                                           maxm86161_bpf_t *pBPF);
static int32_t maxm86161_hrm_frame_process(maxm_hrm_handle_t *handle,
                                           int16_t *heart_rate,
                                           int32_t *HeartRateInvalidation,
...
static int32_t maxm86161_hrm_sample_process(maxm_hrm_handle_t *handle, uint32_t hrm_ps,
                                            ...
                                            uint32_t SpO2_PS_IR,
                                            mamx86161_hrm_data_t *hrm_data);
static int32_t maxm86161_hrm_spo2_frame_process(maxm_hrm_handle_t *handle,
                                                int16_t *SpO2,
                                                int32_t *HeartRateInvalidation,
                                                mamx86161_hrm_data_t *hrm_data);
static int32_t maxm86161_hrm_identify_part(uint8_t *part_id);
...
/* Q16. fs=79; [BPF_b,BPF_a] = cheby2(3,35, [18, 500]/60/fs*2);
 * 0dB at 45BMP. -1dB at 160BPM, -6dB at 200BPM. No overflow.
 */
static int32_t bpf_60hz[] = {
...
static const int16_t num_of_lowest_hr_cycles_min = 300; // 3.00*60/45BPM=4.0s, Min(Initial) #(x100) of lowest-HR cycle. The longer the frame scale is, more accurate the heart rate is.
static const int16_t num_of_lowest_hr_cycles_max = 500; // 5.00*60/45BPM=6.7s, Max(Final) #(x100) of lowest-HR cycle. The longer the frame scale is, more accurate the heart rate is.
static const int16_t f_low = 45;              // (bpm), min heart rate.
static const int16_t f_high = 200;            // (bpm), max heart rate.
static const uint16_t hrm_raw_min_thresh = 6000; // HRM PS threshold for validation (this depends on the LED active current: 16000 for current=0xF(359mA))
...
static const uint16_t spo2_dc_to_acpp_ratio_high_thresh = 600; // 1/60=1.67%, 1/100=1%. DC to ACpp ratio threshold for validation. Normally, <250 for finger. <700 for wrist.
...
#define checkParamRange(param_value, min, max, param_number)      \
...
int32_t maxm86161_hrm_run(maxm_hrm_handle_t *handle)
{
  ...
  //handle->device_config=default_maxim_config;
  //maxm86161_init_dev(default_maxim_config);
  maxm86161_init_dev(*handle->device_config);
  printf("maxm86161_hrm_init_measurement_parameters.\r\n");
                                              ...
                                              maxm86161_hrm_irq_sample_t *samples)
{
  ...
  error = maxm86161_hrm_sample_process(handle,
                                       *ppg_ptr[handle->hrm_ps_select],
                                       *ppg_ptr[handle->spo2_red_ps_select],
                                       *ppg_ptr[handle->spo2_ir_ps_select],
                                       hrm_data);
                              ...
                              mamx86161_hrm_data_t *hrm_data)
{
  ...
  for (i = 0; i < numSamples; i++) {
      ...
      if ((handle->hrm_dc_sensing_flag == HRM_DC_SENSING_START)
        ...
        *hrm_status |= MAXM86161_HRM_STATUS_FINGER_OFF;
      ...
      } else if (handle->hrm_dc_sensing_flag
                 == HRM_DC_SENSING_CHANGE_PARAMETERS) {
        ...
        if ((handle->heart_rate_invalidation_previous
            ...
            == MAXM86161_HRM_STATUS_SUCCESS) {
          for (channel = 0; channel < 3; channel++) {
            // sum the raw data for later averaging.
            handle->hrm_agc[channel].raw_ppg_sum += samples.ppg[channel];
            handle->hrm_agc[channel].raw_ppg_count++;
          }
        }
        maxm86161_hrm_perform_agc(handle);
  #endif
        error = maxm86161_hrm_process_external_sample(handle,
                                                      heart_rate,
                                                      SpO2,
                                                      hrm_status,
                                                      hrm_data,
...
static void maxm86161_hrm_perform_agc(maxm_hrm_handle_t *handle)
{
  ...
  for (channel = 0; channel < 3; channel++) {
    if ((handle->hrm_agc[channel].raw_ppg_count >= HRM_NUM_SAMPLES_IN_FRAME)) {
      ...
      average_ppg = handle->hrm_agc[channel].raw_ppg_sum/handle->hrm_agc[channel].raw_ppg_count;
       ...
       * When PPG is out of the range, AGC increase/decrease the LED current by 1. */
      agc_threshold_percent = 20;

      if ((average_ppg < (uint32_t)HRM_DC_WORKING_LEVEL * (100 - agc_threshold_percent) / 100)
          || (average_ppg > (uint32_t)HRM_DC_WORKING_LEVEL * (100 + agc_threshold_percent) / 100)) {
        ...
        /* Signal that the AGC made a change to the current or gain.
         * This flag is cleared in sample process. */
        handle->hrm_agc[channel].agc_flag = 1;
      }
    }
  }
}
                                             ...
                                             maxm86161_hrm_irq_sample_t *sample)
{
  ...
  // If we found the working level for all led
  if ((handle->dc_sensing_finish[0] == true)
  ...
  // If we have tried all the LED currents
  else if (handle->ppg_led_local == 255) {
    handle->hrm_dc_sensing_flag = HRM_DC_SENSING_CHANGE_PARAMETERS;
    for (channel = 0; channel < 3; channel++) {
      // check led not reach HRM_DC_WORKING_LEVEL
      if (handle->dc_sensing_finish[channel] == false) {
...
static int32_t maxm86161_hrm_initialize_buffers(maxm_hrm_handle_t *handle)
{
  ...
  for (i = 0; i < (INTERPOLATOR_L * 2); i++)
    handle->hrm_interpolator_buf[i] = 0;  // Initialized the whole HRM interpolator buffer to 0.
  ...
  handle->bpf_active_flag = (0xff - HRM_BPF_ACTIVE - SpO2_RED_BPF_ACTIVE - SpO2_IR_BPF_ACTIVE);
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_bpf_filtering(maxm_hrm_handle_t *handle,
                                           int32_t ps_input,
                                           int16_t *p_ps_output,
                                           maxm86161_bpf_t *p_bpf)
{
	// Band-pass filter: 32-bit integer-point precision.
	  ...
	  for (i = 0; i < handle->bpf_biquads; i++) {
	    ...
	    /* Update new input sample for next 2nd-order stage.
	     * point p_bpf_b/_a to the next 2nd-order stage. */
	    p_bpf_b += 6;
	    p_bpf_a += 6;
	  }
	  // 0.5=roundup
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_frame_process(maxm_hrm_handle_t *handle,
                                           int16_t *heart_rate,
                                           int32_t *p_heartrate_invalidation,
                                           mamx86161_hrm_data_t *hrm_data)
{
	 int32_t error = MAXM86161_HRM_SUCCESS;
	  int16_t i, j, k, m;
	  // Zero-crossing count and the heart rate estimated from zc_count
	  int16_t zc_count, zc_hr;

	  /* Variables for BPF(PS) min and max.
	   * Initializations are needed to avoid the usage before set. */
	  int16_t hrm_ps_vpp_low = 0, hrm_ps_vpp_high = 0;
	  ...
	  const int32_t hrm_ps_auto_corr_max_threshold = 100000 / 2880 / 4;
	  int16_t heart_rate_x10;
	  // Output index to the circular buffer of the frame sample buffer.
	  int16_t hrm_buffer_out_index;
	  ...
	  if (handle->sample_count >= handle->hr_update_interval) {
	    ...
	    *p_heartrate_invalidation = MAXM86161_HRM_STATUS_SUCCESS;
	    ...
	    if (handle->spo2 != NULL) {
	      // Notify SpO2 to process the SpO2 frame.
	    ...
	    if (handle->hrm_ps_raw_level_count < handle->hr_iframe) {
	    ...
	    } else if ((handle->heart_rate_invalidation_previous
	                & MAXM86161_HRM_STATUS_HRM_MASK)
	      ...
	      for (k = 0; k < handle->hr_iframe * 5 / 100; k++) {
	        if (handle->hrm_sample_buffer[i] != 0) {
	          break;  // Break if non-zero
	        }
	        if (++i == MAX_FRAME_SAMPLES) {
	          i = 0;  // Wrap around
	        }
	      }
	      if (k == handle->hr_iframe * 5 / 100) {
	        *p_heartrate_invalidation |= MAXM86161_HRM_STATUS_FINGER_ON;
	      }
	    }
	    // Skip below if the frame is invalid for heart-rate detection
	    if (*p_heartrate_invalidation == 0) {
	      ...
	      if ((hrm_ps_vpp_low < handle->hrm_ps_vpp_min)
	          || (hrm_ps_vpp_high > handle->hrm_ps_vpp_max)) {
	      ...
	      if (cf_abs_max * cf_abs_max > handle->hrm_ps_crestfactor_thresh
	    ...
	    // Skip if the frame is invalid for heart-rate detection
	    if (*p_heartrate_invalidation == 0) {
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_spo2_frame_process(maxm_hrm_handle_t *handle,
                                                int16_t *SpO2,
                                                int32_t *p_heartrate_invalidation,
                                                mamx86161_hrm_data_t *hrm_data)
{
	  ...
	  if (handle->spo2->spo2_percent == SPO2_STATUS_PROCESS_SPO2_FRAME) {
	    ...
	    i = handle->spo2->spo2_buffer_in_index - handle->hr_update_interval;
	    if (i < 0) {
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_init_measurement_parameters (maxm_hrm_handle_t *handle, int16_t measurement_rate)
{
	int32_t error = MAXM86161_HRM_SUCCESS;
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_get_sample(maxm86161_hrm_irq_sample_t *samples)
{
  int32_t error = MAXM86161_HRM_SUCCESS;
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_identify_part(uint8_t *part_id)
{
 ...
 *****************************************************************************/
int32_t maxm86161_hrm_sample_process(maxm_hrm_handle_t *handle,
                                            uint32_t hrm_ps,
                                            uint32_t spo2_ps_red,
                                            uint32_t spo2_ps_ir,
                                            mamx86161_hrm_data_t *hrm_data)
{
  ...
  if (handle->hrm_agc[0].agc_flag == 1) {
    ...
    for (i = 0; i < (INTERPOLATOR_L * 2); i++)
      handle->hrm_interpolator_buf[i] = handle->hrm_interpolator_buf[i]*hrm_ps / handle->hrm_agc[0].saved_ppg;  // Scale the HRM interpolator data buffer
  ...
  if (handle->hrm_agc[1].agc_flag == 1) {
    ...
    for (i = 0; i < (INTERPOLATOR_L * 2); i++)
      handle->spo2->spo2_ir_interpolator_buf[i] = handle->spo2->spo2_ir_interpolator_buf[i] * spo2_ps_ir / handle->hrm_agc[1].saved_ppg;  // Scale the SpO2 interpolator data buffer
  ...
  if (handle->hrm_agc[2].agc_flag == 1) {
    ...
    for (i = 0; i < (INTERPOLATOR_L * 2); i++)
      handle->spo2->spo2_red_interpolator_buf[i] = handle->spo2->spo2_red_interpolator_buf[i] * spo2_ps_ir / handle->hrm_agc[2].saved_ppg;  // Scale the SpO2 interpolator data buffer
  ...
  for (i = 0; i < (INTERPOLATOR_L * 2 - 1); i++)
    handle->hrm_interpolator_buf[i] = handle->hrm_interpolator_buf[i + 1];  // Use memmove() instead for efficiency
  ...
  for (i_interpolator = 0; i_interpolator < handle->hrm_interpolator_factor; i_interpolator++) {
    if (i_interpolator == 0) {
      // Don't touch hrm_ps and spo2_ps_red/IR if handle->hrm_interpolator_factor==1;
  ...
  if (handle->spo2 != NULL) {
    ...
    maxm86161_hrm_bpf_filtering(handle,
                                ...
                                &handle->spo2->spo2_red_ac_sample_buffer[handle->spo2->spo2_buffer_in_index],
                                &handle->spo2->spo2_red_bpf);
    // Add the BPF(PS3) in the framed sample circular buffer.
    maxm86161_hrm_bpf_filtering(handle,
                                (int32_t)spo2_ps_ir_scaled,
                                &handle->spo2->spo2_ir_ac_sample_buffer[handle->spo2->spo2_buffer_in_index],
    ...
    if (++handle->spo2->spo2_buffer_in_index == MAX_FRAME_SAMPLES)
      handle->spo2->spo2_buffer_in_index = 0;  // Wrap around
  }

  /* Count how many samples are valid since the last Finger-Off.
   * Finger-On detection uses the raw PS and its detection threshold. */
  if (hrm_ps < HRM_DC_SENSING_WORK_LEVEL * 0.5)
    handle->hrm_ps_raw_level_count = 0;
  else {
    if (++handle->hrm_ps_raw_level_count > MAX_FRAME_SAMPLES)
      handle->hrm_ps_raw_level_count--;   // limit the count
  }
  handle->sample_count++;
  // Copy these variables to hrm_data for display.
  if (hrm_data != 0) {
